...
def get_item_dim() -> PySparkDF: #  we take only unique combination of item nbr , dept nbr , sb class such that dept nbr is present in recommended list
    sql_query = f"""
        SELECT 
          DISTINCT
          MDS_FAM_ID as system_item_nbr,
...
def get_inv_dept(inv: PySparkDF) -> PySparkDF: # only inventory table is used and not item dim
    inv_dept = inv.withColumn('club_nbr', F.col('club_nbr').cast('Integer')) \
        .withColumn("date", F.date_format(F.col("snapshot_date"), "yyyy-MM-dd")) \ # date is snapshot date
        .withColumn('inventory_qty', # we create inventory qty here
                    F.when((F.col("dept_nbr").isin([42, 44, 57])), F.col('onsite_onhand_qty') + F.col('on_order_qty')) \
                    .otherwise(F.col('onsite_onhand_qty'))) \
        .withColumn('inventory_qty', F.col('inventory_qty').cast('Integer'))
    return inv_dept

def get_first_level_features(raw_sales: PySparkDF, inv_dept: PySparkDF, item_dim: PySparkDF) -> PySparkDF: 
    item_cat = item_dim.select('system_item_nbr', 'dept_nbr', 'subclass_nbr', 'item_nbr').distinct()# no need as we have select distict item nbr in item_dm
    # filtering out only those item nbr that have dpt nbr in recommended list, now agg at item club and date of placing on shelf, now taking max snap shot date
    item_onshelf = inv_dept.filter(F.col('dept_nbr').isin(CATS)).groupBy('system_item_nbr', 'club_nbr', 'item_on_shelf_date_dt').agg(F.max('snapshot_date').alias('max_snapshot_date'))
    ...
    day_sales_base = raw_sales.filter(F.col('unit_qty') > 0) \
        .groupBy('store_nbr', 'scan_id', 'visit_date') \# group by item club and date we get agg columns - unit sold count , sale amt , unit price amt 
        .agg(F.sum('unit_qty').cast('decimal(18,2)').alias('unit_sold_cnt'), 
             F.sum('retail_price').cast('decimal(18,2)').alias('sale_amt'),
             (F.sum('retail_price')/F.sum('unit_qty')).cast('decimal(18,2)').alias('unit_price_amt')) \
        .withColumnRenamed('store_nbr', 'club_nbr') \
        .withColumnRenamed('visit_date', 'date') \
        .withColumnRenamed('scan_id', 'system_item_nbr') \
        .join(item_cat, on=['system_item_nbr'], how='inner') \# this shall remove orders for those items which have dept nbr not present in recommmended list
        .join(inv_dept.drop('dept_nbr', 'subclass_nbr', 'item_nbr', 'item_on_shelf_date_dt'), on=['system_item_nbr', 'date', 'club_nbr'], how='left') \ # we add inventory qty, date ,on sit & on hand qty, already got dept nbr , item nbr, , sub class nbr from item cat and item onshelf date will be coming from item onshelf
        .withColumn('unit_sold_cnt', F.coalesce('unit_sold_cnt', F.lit(0))) \
        .withColumn('inventory_qty', F.coalesce('inventory_qty', F.lit(0)))  
    
    join_cond = [day_sales_base.system_item_nbr == item_onshelf.mds_fam_id, day_sales_base.club_nbr == item_onshelf.store_nbr, day_sales_base.date >= item_onshelf.item_on_shelf_date_dt, day_sales_base.date <= item_onshelf.max_snapshot_date] # to make sure that for  an order at item club date level there is also existing inventory  availabe on that date     day_sales_base = day_sales_base.join(item_onshelf, how='left', on=join_cond) 
        day_sales_base = day_sales_base.join(item_onshelf, how='left', on=join_cond) # join cond - from here item_on_shelf col is added to days sales base table
    ...
    logs = day_sales_base.filter(F.col('date') >= str(date.today() - timedelta(days=485))).select( # here we select unique comb of mds , clb ,dept ,itme for which items were sold in past 485 days
        'mds_fam_id', 'club_nbr', 'department_nbr', 'subclass_nbr', 'item_nbr').distinct()
    logs = logs.withColumn('date', F.lit(TODAY).cast('string')) # in selected comb above we provide todays date becoz these recommendation are for present date.
    day_sales_base = day_sales_base.join(logs, on=['mds_fam_id', 'club_nbr', 'date', 'department_nbr',
                                                   'subclass_nbr', 'item_nbr'], how='outer') # for today's date we shall have additional rows created if the recommended item is not sold 

    day_sales_base = day_sales_base.withColumn('unit_sold_cnt', F.coalesce('unit_sold_cnt', F.lit(0))) # since we have added rows becoz of logs for which no sale happened , we need to impute units sold with 0
    ...
    # Get the past 4 weeks avg pricesales , units sold for each 1 to 4 weeks
    for i in range(12):
        if i <= 3:
            col_name_suffix = str(4 - i) + '_week_back_cnt'
            price_col_name_suffix = str(4 - i) + '_week_back_amt'
        else:
        ...
        if i <= 3:
            ...
            units_winSpec = Window.partitionBy('club_nbr', 'mds_fam_id').orderBy('unix_time').rangeBetween(86400,
                                                                                                                7 * (
                                                                                                                        i - 3) * 86400)
        day_sales_base = day_sales_base.withColumn('unit_sold_' + col_name_suffix, F.sum('unit_sold_cnt').over(units_winSpec)) # units sold for both 1 to 4  weeks backs and 1 to 8 weeks future count
...
if __name__ == '__main__':
    ...
    # Output tables
    output_table = generate_table_name(project_id, "markdown_vn57vog", "inclub_first_level_vn57vog")
